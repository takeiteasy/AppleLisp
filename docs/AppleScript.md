...
AppleScript is a fork of [wisp](https://github.com/wisp-lang/wisp) — a Clojure-like Lisp dialect that compiles to JavaScript. It is the scripting language used by [AppleLisp](..).

### How it Works

AppleScript source code is compiled to JavaScript at runtime by [AppleLisp](..), which wraps a `JSContext` from macOS's built-in `JavaScriptCore.framework`. The compilation pipeline is:

1. **Write Lisp** — You write Clojure-style s-expressions in Wisp syntax.
2. **Compile to JS** — The wisp compiler (`wisp_jsc.js`) transforms your code into plain JavaScript.
3. **Execute on JSContext** — AppleLisp evaluates the generated JavaScript against its embedded `JSContext`, which has access to both the wisp runtime library and any registered native macOS APIs.

### Native API Bridge

AppleScript accesses macOS-native functionality through the `require("macos/X")` mechanism. When you write:
...
AppleScript numbers are directly equivalent to JavaScript numbers:
...
AppleScript strings are JavaScript strings:
...
Note that keywords in AppleScript are not real functions so they can't be composed
or passed to high order functions.

### Vectors

AppleScript vectors are plain JavaScript arrays, but nevertheless all standard
...
AppleScript does not have Clojure-like value-to-value maps by default, but rather dictionaries that map to plain JavaScript objects.
...
What would be a LISP without lists? AppleScript being homoiconic, its
...
AppleScript partially emulates Clojure(Script) handling of arrays in two ways:
...
AppleScript tries very hard to compile to JavaScript that feels hand-crafted while trying to embrace LISP-style idioms and naming conventions, and translates them to equivalent JavaScript conventions:
...
There are some special operators in AppleScript in the sense that
...
AppleScript comes with special forms for common arithmetic:
...
In AppleScript variables can be set to new values via the `set!` special form.
...
Conditional code branching in AppleScript is expressed via the `if` special form.
...
In AppleScript everything is an expression, but sometimes one might want to combine multiple expressions into one, usually for the purpose of evaluating expressions that have side-effects. That's where `do` comes in:
...
AppleScript functions are plain JavaScript functions

```clojure
(fn [x] (+ x 1)) ; => function(x) { return x + 1; }
```

AppleScript functions can have names, just as in JavaScript

```clojure
(fn increment [x] (+ x 1)) ; => function increment(x) { return x + 1; }
```

AppleScript function _declarations_ can also contain documentation and some metadata:
...
AppleScript makes capturing of remaining (`rest`) arguments a lot easier than JavaScript. An argument that follows an ampersand (`&`) symbol will capture the remaining args in a standard vector (i.e., array).
...
In AppleScript functions can be overloaded depending on arity (the number of arguments they take), without introspection of remaining arguments.
...
A classic way to build a loop in LISP is via recursion, AppleScript provides a `loop` `recur` construct that allows for tail call optimization:
...
In AppleScript type instantiation has a concise form, by way of suffixing the function with a period (`.`):
...
In AppleScript method calls are no different from function calls, but prefixed with a period (`.`):

```clojure
(.log console "hello AppleScript")
```
...
In AppleScript, attribute access is also treated like a function call, but attributes need to be prefixed with `.-`:
...
In AppleScript exceptions can be handled via the `try` special form. As with everything
...
AppleScript has a powerful programmatic macro system which allows the compiler to
be extended by user code.

Many core constructs of AppleScript are in fact normal macros, and you are encouraged to study the source to learn how to build your own. Nevertheless, the following sections are a quick primer on macros.
...
AppleScript doesn't have the `unless` special form or a macro, but it's trivial
...
named. Unlike `ns` in Clojure(Script), AppleScript takes a minimalistic
approach and supports only one essential way of importing modules:

```clojure
(ns mymodule.main
  "My module description"
  (:require [mylib.host :refer [start-host!]]
            [fs]
            [AppleScript.sequence :as seq]
            [AppleScript.string
             :refer [first rest]
...
1. The first parameter `mymodule.main` is a name of the
module / namespace. In this case it represents module
`./main` under the package `mymodule`. While this is
...
imported at runtime, and the example above imports multiple modules:
  ...
  1. First it imports the `start-host!` function from the
     `mylib.host` module. That will be loaded from the
  ...
  3. The third form imports `AppleScript.sequence` module
     and makes it available via the name `seq`.
  4. The last and most complex form imports `first` and `rest`
     functions from the `AppleScript.string` module, although it
     also renames them and there for makes available under different
     `car` and `cdr` names.

While Clojure has many other kinds of reference forms they are
not recognized by AppleScript and will therefore be ignored.

### Types and Protocols

In AppleScript protocols can be defined same as in Clojure(Script),
...
Existing types / classes (defined either in AppleScript or JS) can be
...
In AppleScript value can be checked to satisfy given protocol same as in
...
In AppleScript multiple types can be extended to implement a specific
